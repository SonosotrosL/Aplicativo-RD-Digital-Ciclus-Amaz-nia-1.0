...
export const Dashboard: React.FC<DashboardProps> = ({ rds, currentUser, onUpdateStatus, onEditRD, onDeleteRD }) => {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "ID", "Data", "Hora", "Supervisor", "Equipe Supervisor", "Encarregado", "Equipe Encarregado", "Base", "Turno", "Status",
    ...
    const dataRows = filteredRDs.map(rd => {
      ...
      return [
        rd.id, dateStr, timeStr, rd.supervisorName || '', rd.supervisorTeam || '', rd.foremanName, rd.foremanTeam || '', rd.base || '', rd.shift || '', rd.status,
