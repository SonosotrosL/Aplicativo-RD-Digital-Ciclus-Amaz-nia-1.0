...
export const RDForm: React.FC<RDFormProps> = ({ currentUser, onSave, onCancel, existingData }) => {
  ...
  const [selectedForemanId, setSelectedForemanId] = useState<string>(existingData?.foremanId || '');
  const [availableForemen, setAvailableForemen] = useState<User[]>([]);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const foremen = allUsers.filter(u => u.role === UserRole.ENCARREGADO);
      setAvailableForemen(foremen);

      // Auto-select roles based on current user
      if (currentUser.role === UserRole.SUPERVISOR && !existingData) {
        setSelectedSupervisorId(currentUser.id);
      }
      if (currentUser.role === UserRole.ENCARREGADO && !existingData) {
        setSelectedForemanId(currentUser.id);
      }
  ...
  const fetchNearbyStreets = async (lat: number, lng: number, currentStreetName: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Map street name to its closest distance found
        const streetDistances = new Map<string, number>();

        data.elements.forEach((el: any) => {
          if (el.tags && el.tags.name && el.center) {
            const name = el.tags.name;
            // Simple Euclidean distance approximation (sufficient for sorting nearby)
            const dlat = el.center.lat - lat;
            const dlng = el.center.lon - lng;
            const distSq = dlat * dlat + dlng * dlng;

            if (!streetDistances.has(name) || distSq < streetDistances.get(name)!) {
              streetDistances.set(name, distSq);
            }
          }
        });
        ...
        const sorted = Array.from(streetDistances.entries())
          .filter(([n]) => {
            const nameLower = n.toLowerCase();
            // Filter out the current street itself to avoid redundancy
            return !currentNorm || !nameLower.includes(currentNorm);
          })
          .sort((a, b) => a[1] - b[1]) // Sort by distance (closest first)
          .map(([n]) => n); // Extract names
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Determine Foreman User
    let foremanUser: User | undefined;
    if (currentUser.role === UserRole.ENCARREGADO) {
      foremanUser = currentUser;
    } else {
      foremanUser = availableForemen.find(u => u.id === selectedForemanId);
      // Fallback: If no foreman selected but current user is one (shouldn't happen with UI logic but for safety)
      if (!foremanUser && currentUser.role === UserRole.ENCARREGADO) foremanUser = currentUser;
    }

    if (!foremanUser) {
      alert("Erro: Encarregado da Equipe não identificado. Por favor selecione um.");
      return;
    }
    ...
    const rdToSave: RDData = {
      ...
      foremanName: foremanUser.name,
      foremanRegistration: foremanUser.registration,
      foremanTeam: foremanUser.team || currentUser.team, // Use selected foreman's team, fallback to current
      supervisorId: selectedSupervisorId,
      supervisorName: sup?.name || '',
      supervisorTeam: sup?.team || '',
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden mb-10">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8">
        ...
        {/* Supervisor Selection: Explicitly select prompt if not a Supervisor, OR if admin wants to override */}
        ...
        {/* Foreman Selection: Show if current user is NOT an Encarregado (e.g. Supervisor filling for someone else) */}
        {(currentUser.role !== UserRole.ENCARREGADO || existingData) && (
          <section className="bg-blue-50 p-4 rounded border border-blue-200">
            <label className="block text-sm font-bold text-blue-800 mb-2 flex items-center gap-2"><Users className="w-4 h-4" /> Encarregado da Equipe</label>
            <select required value={selectedForemanId} onChange={e => setSelectedForemanId(e.target.value)} className="w-full p-2 border border-blue-300 rounded bg-white text-gray-700 focus:ring-2 focus:ring-ciclus-500 outline-none">
              <option value="">Selecione o Encarregado...</option>
              {availableForemen.map(f => <option key={f.id} value={f.id}>{f.name} (Mat: {f.registration}) - {f.team || 'Sem Equipe'}</option>)}
            </select>
          </section>
        )}
        ...
        <section className="border-t border-gray-100 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <div className="md:col-span-1">
              ...
              {nearbyStreets.length > 0 && (
                <div className="mt-2 animate-in fade-in bg-blue-50 p-2 rounded border border-blue-100">
                  <p className="text-[10px] text-blue-700 mb-1 flex items-center gap-1 font-bold"><MapIcon className="w-3 h-3" /> Toque para adicionar referência:</p>
                  <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto custom-scrollbar">
                    {nearbyStreets.map((st, i) => {
                      const isSelected = selectedPerimeterStreets.includes(st);
                      return (
                        <button
                          type="button"
                          key={i}
                          onClick={() => handleAddPerimeterStreet(st)}
                          className={`text-[10px] px-2 py-1 rounded border transition-colors shadow-sm ${isSelected ? 'bg-blue-600 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-blue-100 hover:text-blue-800'}`}
                        >
                          {st}
                        </button>
                      );
