...
export const generateDailyReportSummary = async (rds: RDData[]): Promise<string> => {
  try {
    ...
    if (error.status === 429 || error.message?.includes('429') || error.message?.includes('quota')) {
        return "⚠️ Limite de uso da IA excedido temporariamente (Erro 429).";
    }
  ...
  try {
    const prompt = `
      ...
      **Eficiência por Supervisor:**
      ...
      3. **Ação Estratégica**: O que deve ser feito para melhorar a média diária.
    ...
    if (error.status === 429 || error.message?.includes('429') || error.message?.includes('quota')) {
        return "⚠️ Cota de IA excedida (Erro 429).";
    }
